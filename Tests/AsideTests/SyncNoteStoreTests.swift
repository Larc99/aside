...
final class SyncNoteStoreTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        folder = FileManager.default.temporaryDirectory
            .appendingPathComponent("AsideSync-\(UUID().uuidString)", isDirectory: true)
    ...
    /// The app was renamed from EdgeNotes to Aside. Sync-folder files live in
    /// the user's own folder, not the app container, so they outlive the
    /// rename and must still load.
    ...
    func testUpsertDoesNotReplaceASymlink() async throws {
        ...
        let target = FileManager.default.temporaryDirectory
            .appendingPathComponent("AsideTarget-\(UUID().uuidString).md")

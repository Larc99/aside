...
enum KeyStoreError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .keychainUnavailable:
            return """
            Aside could not reach your login keychain, so it cannot unlock \
            your notes. Minting a new key here would make every existing note \
            unreadable, so Aside stopped instead. Unlock your keychain and \
            open Aside again.
            """
        }
    }
...
enum KeyStore {
    private static let service = "Aside"

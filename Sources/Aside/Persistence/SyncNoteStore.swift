...
/// a symlink, a device node. Aside never writes over or deletes those:
...
actor SyncNoteStore: NoteStore {
    ...
    nonisolated private static func makeWatcher(
        ...
        let source = DispatchSource.makeFileSystemObjectSource(
            ...
            queue: DispatchQueue(label: "Aside.sync-folder-watcher", qos: .utility)
        )
    ...
    private static let formatKey = "aside"
    private static let legacyFormatKey = "edgeNotes"
    ...
    nonisolated private static func parse(_ data: Data) -> Note? {
        ...
        for line in header {
            ...
            switch key {
            // Files written before the app was renamed carry the old key.
            // They live in the user's own sync folder, outside the app
            // container, so they survive the rename and must keep loading.
            case formatKey, legacyFormatKey: sawFormat = Int(value) == formatVersion

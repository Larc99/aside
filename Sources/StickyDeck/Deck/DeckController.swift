...
final class DeckController {
    ...
    private func relayout(animated: Bool, reason: RelayoutReason = .dataChange) {
        ...
        if ProcessInfo.processInfo.environment["STICKYDECK_DEBUG_FAN"] != nil || ProcessInfo.processInfo.environment["STICKYDECK_DEBUG_EXPAND"] != nil || ProcessInfo.processInfo.environment["STICKYDECK_DEBUG_AUTOSAVE"] != nil {
            NSLog("StickyDeck relayout state=\(state) count=\(count) animated=\(animated)")
        }
